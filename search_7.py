...
def search(input_search_locations=[], retry=0, z=0):
                        ...
                        if ('business_status' in response['results'][0]):  # 'business_status'가 없는 경우 분류
                            if (response['results'][0]['business_status'] != 'CLOSED_PERMANENTLY'):  # 폐업인 경우 제외
                                ...
                                if ('photos' in response['results'][0]):  # 'photos'가 아예없는 경우 제외
                                    # 사진 요청
                                    photo_reference = response['results'][0]['photos'][0][
                                        'photo_reference']  # 'photos'중 첫번째꺼
                            ...
                            for i in range(len(res_lo['results'])):
                                ...
                                if ('rating' in res_lo['results'][i]):
                                    ...
                                    if ('business_status' in res_lo['results'][i]):  # 'business_status'가 없는 경우 분류
                                        if (res_lo['results'][i][
                                            'business_status'] != 'CLOSED_PERMANENTLY'):  # 폐업인 경우 제외
                                ...
                                if ('business_status' in response_blank['results'][0]):  # 'business_status'가 없는 경우 분류
                                    if (response_blank['results'][0][
                                        ...
                                        if ('photos' in response_blank['results'][0]):  # 'photos'가 아예없는 경우 제외
                                            # 사진 요청
                                            photo_reference = response_blank['results'][0]['photos'][0][
                                                'photo_reference']  # 'photos'중 첫번째꺼
                                            photo_url = f"https://maps.googleapis.com/maps/api/place/photo?maxwidth=1600&photoreference={photo_reference}&key={api_key}"
                                            response_photo = requests.get(photo_url)
                                            res_photo = response_photo.url
                                        ...
                                        # 좌표
                                        search_location_lat = (
                                        response_blank['results'][0]['geometry']['location']['lat'])
                                        search_location_lng = (
                                        response_blank['results'][0]['geometry']['location']['lng'])
                                        destination.append(search_location_lat)
                                        destination.append(search_location_lng)
...
# TEST
# res_sol = search(
#     ['Great Barrier Reef(Queensland, Australia)', 'Uluru-Kata Tjuta National Park(Northern Territory, Australia)',
#      'Port Douglas(Queensland, Australia)', 'Melbourne(Victoria, Australia)'])
...
# res_sol = search(['Tokyo(tokyo prefecture, Japan)', 'Kyoto(Kansai Region, Japan)', 'Osaka(Kansai Region, Japan)',
#                   'Hiroshima(Chugoku Region, Japan)', 'Nara(Kansai Region, Japan)'])
# print(res_sol)
# res_sol = search(['Tokyo', 'Kyoto', 'Osaka', 'Hiroshima', 'Nara'])
# res_sol=search(['Hiroshima(Chugoku Region, Japan)'])
# res_sol = search(['Machu Picchu(Cusco Region, Peru)', 'Galapagos Islands(Ecuador)', 'Rio de Janeiro(Rio de Janeiro State, Brazil)', 'Salar de Uyuni(Potosi, Bolivia)', 'Torres del Paine National Park(Magallanes and Chilean Antarctica, Chile)'])
# print(res_sol)
# res_sol = search(['Machu Picchu', 'Galapagos Islands', 'Rio de Janeiro', 'Salar de Uyuni', 'Torres del Paine National Park'])
# print(res_sol)
# res_sol = search(['Don Quixote(Tokyo, Japan)', 'Don Quixote'])
# print(res_sol)
# print(res_sol)
# ['Tokyo(Kanto Region, Japan)', 'Kyoto(Kansai Region, Japan)', 'Osaka(Kansai Region, Japan)', 'Hiroshima(Chugoku Region, Japan)', 'Nara(Kansai Region, Japan)']
# ['Sydney Opera House(Sydney, New South Wales, Australia)', 'Great Barrier Reef(Queensland, Australia)', 'Uluru-Kata Tjuta National Park(Northern Territory, Australia)', 'Port Douglas(Queensland, Australia)', 'Melbourne(Victoria, Australia)']
